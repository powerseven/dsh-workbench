...
| 约束 | 含义 |
|---|---|
| 面板形状 | 宿主内**又宽又矮**：770–1430px 宽、676px 高。稀缺的是**高度**，不是宽度（与手机「宽度稀缺」相反） |
| 字重 | 宿主只给 400 / 500 两档 |
...
手机稀缺**宽度**，本面板稀缺**高度**。能搬的不是「响应式断点」，而是三条**密度纪律**：

1. **底部常驻主操作**：＋捕获 / ↑发送 / ✎编辑 已聚在输入条——同 Todoist、Google Tasks 的「底部常驻输入框」，单点可达。
2. **用筛选/视图收敛无限列表**：芯片 + 视图切换把全量摊平成「今日/本周」——同 To Do 的 My Day、Things 的 Today；本轮 Upcoming「空天不占行」正是小屏最忌「空白列表占位」的反面。
3. **顺延/置顶信号分离**：TeuxDeux 的 rollover → 已落成「逾期单独置顶」。
...
- [ ] **TeuxDeux 顺延**：未完成项是否要「滚」到下一天/下周（而非只置顶逾期段）
- [ ] **Sunsama shutdown 仪式**：每日收尾时把没做完的重新规划，而非留着堆
- [ ] **Linear 命令面板**：`/` 或 Cmd+K 跳转 + 快速记一条（纯客户端，不改数据模型）
- [ ] **Superlist / Google Tasks 留白纪律**：冗余装饰再砍一层
