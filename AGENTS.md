...
- **快捷问法最多三条**（`QUICK_ASKS`）：那条提示行在手机 390px 宽放不下四个芯片 +
...
| 产出 | 是什么 | 人怎么确认 |
| --- | --- | --- |
| `tasks` | **新建**待办（拆解 / 从图里读） | 草稿卡 → 归入候选 / 选项芯片 → 详见表单 |
...
改动卡是 `/node-set`。原来三条路都要过表单（芯片把人送进详情页、还得再点保存），
而「AI 草稿必经表单」那条纪律的本意是「AI 不替你决定」，**点这一下就是人的决定**；
想改一改再存的走芯片那条路。行首的「全部按首选建议加入」也**真的全部落库**了——
...
- **数字全部来自 `summarize()` 里那份和面板筛选芯片同源的派生量**（`filterCounts`），
