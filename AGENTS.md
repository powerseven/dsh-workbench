...
筛选按钮在看板里同样生效（`boardColumns` 内部复用 `focusList` 的口径）。
...
**浮层是「一次输入」，不是一本对话记录**（用户原话：「下次再点开的时候应该自动清空
之前那个任务，不然话又堆在一起；每次点开那个应该是一个全新的」）：

- **打开 = 全新一屏**（`openFab`）：清掉输入框、贴的图、上一轮问答、上一轮拆出来的
  草稿卡。不清的话最直接的毛病是——上次没发出去的那句话还躺在输入框里，接着用输入法
  说话会**接在后面**。代价要认：**没处理的 AI 草稿也会一起清**（那是建议不是数据，
  真要一次处理多条就用草稿区那颗「全部按首选建议加入」）。
- **写完了就自己收起**：纯输入框「记入收件箱」成功后、以及把草稿交给详情表单时，
  都 `setFabOpen(false)`——用户不该为了关掉一个已经结束的输入窗口再点一次。交给表单
  那次尤其要关：表单是**整块替换面板**、浮层本来就不渲染，留着 `fabOpen` 只会让
  「保存完回到树上」时浮层**又自己弹回来**。
- **关闭入口只有一个**：标题行那颗 ✕（`dsh-wb-fabclose`）。快捷行里原来还有一颗
  「收起」，与它调的是同一个 `setFabOpen(false)`——同一个动作两个控件，用户会先想
  「这俩有区别吗」，而这个问题对他零价值。留标题行那颗（关闭面板的常规位置）。
  它的点击区靠**带上下文的选择器**放大（`.dsh-wb-fabrow .dsh-wb-fabclose`：两个类压过
  一个类，**与源码顺序无关**）——写成单类就得赌顺序，而 `.dsh-wb-icon` 的 padding
  也是单类、又写在后面，第一版就是这么「规则看着在、量出来还是 30×22」的。
- **快捷问法最多三条**（`QUICK_ASKS`）：那条提示行在手机 390px 宽放不下四个按钮 +
  「清空」，折成两行会把浮层整个顶高一行（真机反馈：「3 条就好了，4 条就变成两行了」）。
  想加第四条，先回去量宽度。

**这根输入框是「助手的命令行」，不是「新建待办的表单」**（用户原话：「我叫你干什么，
你应该都要干什么，然后进行回应，然后让我再确认」「你进行一个决策，把意见反馈出来……
意见应该清晰明了、可以让我选择，我选择之后你就按我的意思去做」）：

一次提交回**三种产出**，都是**建议**，都要人过一遍才落库：

| 产出 | 是什么 | 人怎么确认 |
| --- | --- | --- |
| `tasks` | **新建**待办（拆解 / 从图里读） | 草稿卡 → 归入候选 / 选项按钮 → 详见表单 |
| `edits` | **改动已有的**（截止 / 重要程度 / 备注 / 标题 / 归属 / 状态 / 负责人 / 周期） | 改动卡（写明「旧 → 新」+ 可选项）→ **详见表单**（走的还是 `/node-set`） |
| `merges` | **合并已有的**（keep 留下、fold 并进去；`mode` 决定 fold 最后去哪，见下） | 合并卡（**明写 fold 最后去哪**：删掉 / 挪成子项）→ 按下即按既有写入口依次执行 |

**merges 有两种 mode，差别在「被并的那几条最后去哪」**（用户原话：「我要的就是要把
一些任务进行合并，然后作为计划，然后其他的作为它的子计划。但是好像现在它说不支持。
我现在要做的就是这个，它要支持，然后给我一个确认。」）：

| mode | 谁的意图 | fold 的去向 | 卡片上的固定一行 |
| --- | --- | --- | --- |
| `merge`（默认） | 「这两条是一件事、重复了」 | 子项/证据/关联并进 keep，然后**删掉** | 会删掉：X、Y |
| `children` | 「把这几条归到一个计划下面」「其他作为子任务/子计划」 | **一条都不删**，每条挪到 keep 下面当子项（keep 因此变成计划） | 不会删任何条目：这 N 条会挪到「X」下面成为子项 |

- **为什么缺省必须是 `merge`**：判错的方向不一样。把 children 误判成 merge **删掉了
  用户的条目**；把 merge 误判成 children 只是多留几条，改完再删也不迟。`normMergeMode`
  认不出来的值一律当 merge。
- **判错的下场必须看得见**：卡上那一行是它的全部风险，所以两种 mode 写不同的固定行
  （不是 tooltip）；✕ 丢掉这条建议即可。
...
**先搬干净再删**，所以任何一步失败都不会丢子项或证据。每一写都自动留档，合错了能回滚。

**每张卡都有一个「就这么办」**（用户原话：「你反馈出来的东西没有可以让我选择确定，
然后确定之后你就帮我做」）：一次点击 = **一次写入**，不经过表单——草稿卡是 `/node-add`，
改动卡是 `/node-set`。原来三条路都要过表单（按钮把人送进详情页、还得再点保存），
而「AI 草稿必经表单」那条纪律的本意是「AI 不替你决定」，**点这一下就是人的决定**；
想改一改再存的走按钮那条路。行首的「全部按首选建议加入」也**真的全部落库**了——
它原来名字这么写、行为却是「打开第一条的表单让你逐条过」，名不副实（用户就是被它坑住的）。

**同名草稿不是新建，是改动**（用户原话：「我本来就有两条任务是已经存在的了，你现在
做的是要进行一些合并删减，而不是说让我确认再加任务」）：模型经常一边在 reply 里写
「这两条本来就在手上，别当新任务再建一遍」，一边照样把它们塞进 `tasks`。那是**格式**
上的错，提示词治不干净，所以 host 的 `splitExistingTasks` 直接拦：**标题完全相等**
（不做模糊匹配——模糊匹配会把沾边的两条并到一起，那是猜不是判）的草稿**不再放进
tasks**，转成带 `exists: true` 的 edits，卡片上写明「已经在计划里，不用再建」。

**标题重复的，客户端直接给一键合并**（用户原话：「我本来就有两条任务是已经存在的了，
你现在做的是要进行一些合并删减」）：`logic.cjs` 的 `duplicateGroupsOf(plan)` 把
**标题完全相等**（去空白标点、大小写无关，与 host 的 `splitExistingTasks` 同一把尺）
且未完成的分成组——保留**信息最多**的那条（有子项 > 有截止/重要度/备注），其余 fold
进去；保留那条**缺**的字段从重复项里补上（`patch`），否则「只有另一条填了截止」
会静默丢掉。浮层给每组一张合并卡（`local: true`，✕ 只是这次不看）。

**为什么放在客户端**：它纯粹是**计划的派生**（不需要模型、不需要 `/ai-parse`），
放客户端就**刷新即生效**——而 host 半身的任何改动都得重启 `dsh web` 才生效。
这一条对「用户正在手机上迭代」的场景差别很大（真机实测：他的计划里确实有两组
重复，简报报「重复 2」，两张卡各带一颗「按这个合并」）。

三条纪律：

- **模型只给标题，匹配回节点是 host 的活**（`hitByTitle` 一处实现，清单 / 改动 / 合并
  共用）。匹配不上的 **ok=false 照样带回去**，卡片压暗并列出来——AI 抄错了名字必须
  让人看见，而不是「这张卡点了没反应」。
- **字段白名单在 `ai.js` 的 `normEdits` 里，放宽的前提是「一律先过表单」**。
  人确认那一下才是安全边界，不是字段个数；但认不得的键（`metric` / `starred`…）
  一个都不进——模型并不知道它们的语义，放进来等于给它清空任意字段的能力。
- **成功判据要跟着产出一起长**（坑 #25）：`reply / tasks / edits / merges / list`
  **全空**才算失败。「把 X 挪到某计划下」既不需要新任务也不需要回答。

**打开浮层，助手先说话**（用户原话：「我点开它，你就应该要给我所有的一些建议」）：
`aiBriefing()` 给一行「现在： 逾期 2 · 落后 1 · 待核验 3 · 收件箱 4」，点哪个跳哪个筛选。

- **数字全部来自 `summarize()` 里那份和面板筛选按钮同源的派生量**（`filterCounts`），
  不在浮层里重算——重算就会出现「浮层说 2 条、面板说 3 条」而没人知道哪个对。
- **只回答「现在有什么值得动一下」，不给判断**：想要模型的判断，接口就是上面那排快捷
  问法（「我今天该做什么」）——那是**一次点击**的事，不该每次打开都替你花掉一次调用。
- **一行为限**（手机上浮层 366px 宽、还压着键盘，多一行就少一条输入的空间；实测
  一行 24px、浮层总高 148px，输入框仍完整在视口内）。没什么可说时也要说一句
  「眼下没有…」，否则「点开就有建议」会时灵时不灵。
...
**36 条，全文在 [`docs/PITFALLS.md`](./docs/PITFALLS.md)**——这里只留索引，因为指令预算
装不下全文（超了会把本文件尾部截掉，那些「每次都要用」的约定反而看不见）。
动手前扫一眼相关的几条；下面每条的第一个句子就是它的结论：
