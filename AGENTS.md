...
node --test test/*.test.mjs   # 跑测试（254 个，分五层见下）
npm test                      # 构建 + 测试
...
23. **宽容器下的列对齐靠「固定轨道 + 容器查询」，代价是标题列有下限。**
    待办行 / 计划头 / 聚焦行在容器 ≥800px 时变成固定列的 grid（每个槽位一条轨道），
    徽章才成列。flex 右对齐下每个徽章的位置取决于「它右边还有几个徽章」，实测同一
    深度里 taskdue 的列会漂 19px、pri 27px、计划头的 behind 33px——眼睛没法沿一列
    往下扫「哪些逾期了」。四个必须知道的点：

    - **用容器查询，不用媒体查询。** 面板宽度由底部工作台决定（1280 视口下约 1004px），
      和视口宽度不是一回事；拖动工作台边界时视口不变，`@media` 会去问视口、问错人。
    - **阈值 800px 是反推出来的**：固定轨道 + 间隙 + 内边距合计吃掉约 565px，再给标题
      留 18em（13px 下约 19 个汉字）→ 565+234≈800。低于它就切回 flex：宁可徽章参差，
      也不能把标题挤成 135px（实测 700px 时标题全折行，内容总高反而从 531 涨到 631，
      密度的好处全赔进去）。边界实测干净：801px 是 grid、799px 是 flex。
    - **只声明列号、没声明行的元素会按 DOM 顺序参与自动排列，而自动排列的游标只能往前
      走。** 所以只要 DOM 顺序与列号顺序相反（例如把 pri 渲染在 evid 前面），后出现的
      那个就会被甩到**第二行**：列看着没错、行高却从 24px 翻到 48px。CSS 里用
      `grid-row:1` 兜底，`test/client.test.mjs` 另有断言钉住「渲染顺序 = 列号顺序」。
    - **`align-items:baseline` 在多行标题上不能用**：Chromium 会拿**最后一行**的基线当
      行的公共基线，徽章被拉到第二行去（实测差 26px）。改用 `align-items:start` +
      元信息 2px 上边距，落在标题**首行**的视觉中线上（首行行盒 20px、胶囊 16px，
      (20-16)/2=2px，是算出来的不是随手值）。

24. **改「客户端测试」做反证时必须先重建产物（有一个例外）。**
    `test/client.test.mjs` 驱动的是 `lib/` 里的**构建产物**（真组件、真 bundle 工厂），
    所以「故意改坏 `src/` 看断言会不会挂」这套反证，**漏掉 `node scripts/build.mjs`
    就会得出「断言没生效」的错误结论**——本次就这样白跑了一轮。`test/build.test.mjs`
    是混合的：`client` / `host` 两个 bundle 来自 `lib/`（要重建），而它的样式类断言
    （`cssBlock()`、以及「CSS 块之外有没有硬编码颜色」那条）直接读 `src/`，不必重建。
    反证模板：改 `src/` → 产物级断言则构建 → 跑测试 → 复原 `src/` → 再构建。

25. **「样式纪律」断言只覆盖 CSS 块，JS 里的内联样式是盲区。**
    `build.test.mjs` 的「不出现硬编码 hex / rgb」是**切出 `const CSS = [...]` 那一块**来查
    的，所以写在渲染代码里的内联样式照样能溜过去。本次就漏了一处：空态里那句提示文字的
    `style: { color: 'rgba(127,127,127,.95)' }`，一直到做列对齐、逐行读渲染代码时才被发现。
    加任何样式纪律断言时，顺手把 CSS 块**之外**的部分也扫一遍——CSS 块的每一行都以
    引号或 `+` 开头（`'.dsh-wb-x{…}',` / `+ '--wb-col-…',`），所以把它们滤掉剩下的就是
    JS 部分：

    ```sh
    # 输出应当为空；有输出说明 JS 里还埋着硬编码颜色
    grep -nE '#[0-9a-fA-F]{3,8}\b|rgba?\(' src/client/index.js | grep -vE "^[0-9]+:\s*['+]"
    ```

    （只该拦颜色：内联样式里的 `width` / `marginLeft` 是按数据算出来的正常用法。
    另外这条命令会连带滤掉以引号或 `+` 开头的普通 JS 行，属可接受的粗筛。）
