...
- `filed`：`true`（可选布尔），**已纳入工作计划**。收件箱里的顶层待办有两条出路：
  ...
  **零新增工具、零新增路由**）。由此 `inboxOf` 与 `workPlans` 互补：一个顶层节点
  要么在收件箱、要么在工作计划栏，不会两边都出现。
...
- `filed`：`true`（可选布尔），**只对顶层待办有意义**——「纳入工作计划」：
  不作为谁的子项，而以独立条目出现在工作计划栏。`inboxOf` 与 `workPlans`
  由此互补（一个顶层节点要么在收件箱、要么在工作计划栏）。挪到别人下面时
  `appendChild` 清掉它；`normalizePlan` 读盘时对非顶层节点再清一遍——否则
  挪回顶层会凭空回到工作计划栏（静默、且从界面上无从解释）。关掉删键，
  不留 `filed:false`。零新增工具 / 路由：走 `/node-set` 与 `plan_node_set`
  的 `filed` 参数（DEP_PARAMS 通道）。
...
面板最上面常驻一行输入（宿主没模型服务时整块不渲染）。它**既能问也能记**：
